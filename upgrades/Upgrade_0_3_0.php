 ...
 * - user_marketing_consent_histories: 1개 복합 인덱스
 */
class Upgrade_0_3_0 implements UpgradeStepInterface
{
    ...
    private const EXPECTED_INDEXES = [
        'user_marketing_consent_histories' => [
            'idx_mkt_consent_hist_user_created',
        ],
    ];
        ...
        foreach (self::EXPECTED_INDEXES as $table => $indexes) {
            ...
            foreach ($indexes as $indexName) {
                ...
                if (in_array($indexName, $existingIndexes)) {
                    ...
                    $context->logger->warning("[v0.3.0] {$table} 테이블에 {$indexName} 인덱스가 없습니다. 마이그레이션을 실행하세요.");
                }
            }
        }

        $context->logger->info("[v0.3.0] 마케팅 인덱스 검증 완료: {$totalFound}/{$totalExpected}개 확인됨");
    }
}
